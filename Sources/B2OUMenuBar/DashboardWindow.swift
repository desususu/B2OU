    ...
    override func draw(_ dirtyRect: NSRect) {
        ...
        for week in 0..<weeksShown {
            for day in 0..<7 {
                ...
                let y = bounds.height - CGFloat(day + 1) * cellStep
...
class DashboardController: NSObject {
    ...
    private func buildAndShow() {
        ...
        let docView = NSView(frame: NSRect(x: 0, y: 0, width: dashWidth, height: docHeight))
        scrollView.documentView = docView

        let w = dashWidth - dashPad * 2
        var cy = docHeight - dashPad
        ...
        for (i, (value, label)) in statData.enumerated() {
            let x = dashPad + CGFloat(i) * statW

            let numLabel = NSTextField(labelWithString: value)
            numLabel.frame = NSRect(x: x, y: cy + 24, width: statW, height: 30)
            ...
            descLabel.frame = NSRect(x: x, y: cy, width: statW, height: 20)
        ...
        // Adjust document view to used height
        let usedHeight = docHeight - cy + dashPad
        docView.frame = NSRect(x: 0, y: 0, width: dashWidth, height: usedHeight)
